...
import { runCompetitiveIntelligenceOnly } from "@/lib/analysis/pipeline";
...
export async function POST(
  ...
  if (!analysis.reportId || analysis.status !== "completed") {
    return Response.json(
      {
        error:
          "Finish the website analysis before running Competitive Intelligence.",
      },
      { status: 400 },
    );
  }
  ...
  after(() => {
    void runCompetitiveIntelligenceOnly(id).catch((err) => {
      console.error("Competitive Intelligence re-run failed:", err);
    });
  });
